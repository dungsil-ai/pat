...
**문서 반영 대상 변경:**
- 아키텍처 문서에 번역 거부 이슈 자동 종료 흐름 보완: `close-translation-issues` 액션이 다음 번역 실행 시 해결 코멘트와 커밋 정보를 남기고 이슈를 닫는 동작을 `docs/architecture.md`에 명시
...
**문서 반영 대상 변경:**
- 재번역이 필요한 항목과 기존 해시 유지 항목이 섞여 있어도 원본 localization 키 순서를 그대로 유지하도록 번역 파일 병합 순서를 보정
- 타임아웃 중간 저장 시에도 아직 순회하지 않은 기존 번역 키를 함께 유지해 파일 잘림 없이 다음 실행으로 이어지도록 안정성 강화
...
**문서 반영 대상 변경:**
- 업스트림 대시보드 이슈 중복 생성 방지: 워크플로우에 `concurrency` 그룹(`upstream-dashboard`)을 추가하여 동시 실행을 직렬화하고, 동일 제목의 이슈가 여러 개 존재할 경우 번호가 가장 낮은 이슈만 유지하고 나머지 중복 이슈를 자동으로 닫음
- CK3 1.19.0 모드 버전 업데이트: ETC, RICE, Search & Trade Artifact, VIET 모드의 `descriptor.mod`를 CK3 1.19.* 버전에 맞게 갱신

**관련 파일:**
- `.github/workflows/upstream-translation-dashboard.yml`
- `ck3/ETC/mod/descriptor.mod`
- `ck3/RICE/mod/descriptor.mod`
- `ck3/Search & Trade Artifact/mod/descriptor.mod`
- `ck3/VIET/mod/descriptor.mod`
