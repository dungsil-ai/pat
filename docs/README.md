...
**주요 개선 사항 (지난 7일):**
- **ai-sdk.dev 전환**: Gemini 호출을 ai-sdk.dev로 일원화하고 Google AI Studio 토큰 사용, `GEMINI_MODEL`로 모델 선택 지원
- **번역 타임아웃 .env 분리**: `TRANSLATION_TIMEOUT_MINUTES`를 `.env`로 이동, `false`/`0`으로 비활성화 가능
- **업스트림 해시 기반 스킵**: 원본 파일 해시가 유지된 경우 전체 파일 번역을 건너뛰어 실행 시간을 단축
- **모드 병렬 처리 자동화**: 기본 병렬 수를 모드 개수로 설정하고 ETC `upstream/` 하위 폴더를 독립 모드로 병렬 처리
- **번역 거부 자동 대체 워크플로우**: `translation-refused` 이슈에 대해 폴백 번역을 적용하는 전용 워크플로우 추가
- **보안·샌드박스 기본값 개선**: Codex 샌드박스 기본 모드를 `danger-full-access`로 조정하고 child_process 명령 주입 취약점 완화
