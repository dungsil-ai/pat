...
**주요 개선 사항 (지난 7일):**
- **U+200E sanitize 강화**: YAML 파서/직렬화와 번역 sanitize가 Left-to-Right Mark를 제거해 키·값 오염을 방지
- **번역 sanitize 보정 추가**: 변수 마커 뒤의 잘못된 `$[` 패턴을 공백 보정으로 정리
- **수동 음역 파일 지정 지원**: `meta.toml`의 `upstream.transliteration_files`로 파일명/와일드카드 기반 음역 모드 지정
- **음역 파일 변경 자동 무효화**: `transliteration_files` diff를 감지해 영향받는 한국어 파일 해시만 재설정하는 워크플로우/CLI 추가
- **업스트림 번역 미반영 대시보드 추가**: 12시간마다 upstream 태그/커밋을 조회해 미반영 모드를 집계하고 `[대시보드] 업스트림 변경 대비 번역 미반영 현황` 이슈로 자동 게시
- **default 전략 대시보드 정밀화**: 기본 브랜치 전체가 아닌 `upstream.localization` 경로에 영향을 준 최신 커밋만 비교해 미반영 판정을 개선 (`["."]`이면 저장소 전체 추적)
- **github 전략 릴리스 필터 보강**: 최신 공개 릴리스만 사용하고 프리릴리즈/드래프트는 제외
- **GitHub API 인증 기본화**: 번역 워크플로우가 `GITHUB_TOKEN`으로 GitHub API를 인증해 레이트 리밋 영향을 완화
- **GitHub Actions Node 24 정렬**: composite action 런타임을 Node 24로 맞추고 로컬 shared 패키지 해석 시 `NODE_OPTIONS=--preserve-symlinks`를 사용
- **기본 Gemini 모델 문서 정합성**: 코드 기본값에 맞춰 `GEMINI_MODEL` 기본 설명을 `gemini-flash-lite-latest`로 정정
- **태그 비교 안정성 보강**: annotated 태그를 커밋까지 펼쳐 비교하고 동일 참조명 업데이트 시 검증을 추가해 version_strategy 기반 체크아웃 신뢰도 향상
- **CK3 해시 붕괴 대응**: CK3 번역 파일 해시가 손상된 경우에도 해시 스킵/재계산이 깨지지 않도록 복구 로직 보완
...
### 최신 업데이트
- **2026-04-20**: 주간 문서 현행화 (U+200E sanitize, transliteration_files 음역 모드/재무효화 반영)
- **2026-04-13**: 주간 문서 현행화 (GitHub 릴리스 전략, 경로 기반 업스트림 대시보드, 워크플로우 인증/Node 24 반영)
...
**최종 업데이트:** 2026-04-20 | **문서 버전:** 1.4.2
