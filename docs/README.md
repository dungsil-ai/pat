...
**주요 개선 사항 (지난 7일):**
- **번역 키 위치 보존 강화**: 재번역 대기 항목이 섞여 있어도 원본 localization 키 순서를 유지해 diff 노이즈와 검수 혼선을 줄임
- **업스트림 해시 캐시 심볼릭 링크 보호**: `.pat-file-hashes.json`는 일반 파일일 때만 읽고 쓰며, 심볼릭 링크는 경고 후 건너뛰어 덮어쓰기를 막음
- **대소문자 충돌 파일 정리**: 업스트림 localization 파일명이 대소문자만 변경되면 기존 한국어 파일을 정리해 충돌과 stale 파일 잔존을 방지
...
### 최신 업데이트
- **2026-05-18**: 주간 문서 현행화 (재번역 대기 항목 혼재 시 원본 키 순서 보존, 타임아웃 중간 저장 안정성 강화 반영)
- **2026-05-11**: 주간 문서 현행화 (업스트림 해시 캐시 심볼릭 링크 보호, 대소문자 충돌 파일 정리 반영)
- **2026-05-04**: 주간 문서 현행화 (업스트림 대시보드 중복 이슈 정리, CK3 1.19 모드 버전 반영)
...
**최종 업데이트:** 2026-05-18 | **문서 버전:** 1.4.4
