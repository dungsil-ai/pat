...
- 빌드: `pnpm exec jiti scripts/upstream-dashboard.ts` → `upstream-dashboard.md` 생성 (GitHub API/GraphQL로 태그·커밋 비교, annotated 태그 지원)
- 게시: 제목 `[대시보드] 업스트림 변경 대비 번역 미반영 현황`, 라벨 `upstream-dashboard` 로 이슈 생성·갱신
- 내용: 게임/모드별 추적 방식(tag/commit), 기준 버전·최신 버전, 비교 링크, 미반영 수 집계
